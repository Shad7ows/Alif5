...
static AlifSizeT find_emptySlot(AlifDictKeysObject* _keys, AlifHashT _hash) { // 1598
	...
	AlifSizeT ix_ = dictKeys_getIndex(_keys, i);
	for (AlifUSizeT perturb = _hash; is_unusableSlot(ix_);) {
		perturb >>= PERTURB_SHIFT;
		i = (i * 5 + perturb + 1) & mask;
		ix_ = dictKeys_getIndex(_keys, i);
	}
	return i;
}
...
static AlifIntT dictSetDefault_refLockHeld(AlifObject* _d, AlifObject* _key, AlifObject* _defaultValue,
	...
	if (!ALIFUSTR_CHECKEXACT(_key) && DK_IS_USTR(mp_->keys)) {
	...
	if (ALIFDICT_HASSPLITTABLE(mp_)) {
		...
		if (ix_ != DKIX_EMPTY) {
			...
			AlifIntT already_present = value != nullptr;
			if (!already_present) {
...
AlifIntT alifDict_setDefaultRef(AlifObject* _d, AlifObject* _key, AlifObject* _defaultValue,
	AlifObject** _result) { // 4262
	AlifIntT res;
